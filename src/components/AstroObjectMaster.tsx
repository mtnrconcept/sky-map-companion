  ...
  return (
    <div className="flex items-center justify-between py-0.5 text-[11px]">
      <span className="flex items-center gap-1.5">
        ...
        <span className={status === "done" ? "text-foreground" : "text-muted-foreground"}>{name}</span>
      </span>
...
export function AstroObjectMaster({ object, masters, recentJobs, onTriggerStack, isStacking }: Props) {
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          ...
          <Card key={s.label} className={`bg-card/50 border ${s.ok ? "border-green-500/20" : "border-border"}`}>
        ...
        <Card className="bg-card/30 border-dashed">
          ...
          <CardContent className="px-4 pb-3">
            {pipelineLog.steps.map((step, i) => (
              <PipelineStep key={i} name={step.name} status={step.status} result={step.result} />
            ))}
          </CardContent>
        </Card>
      )}
