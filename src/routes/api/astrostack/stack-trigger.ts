 ...
 * Lance un job de stacking pour un objet donné.
 * Sélectionne les meilleures frames qualifiées disponibles.
 */
export const ServerRoute = createServerFileRoute("/api/astrostack/stack-trigger").methods({
  POST: async ({ request }) => {
    ...
    // Sélectionne les meilleures lights qualifiées
    ...
    if (!lights || lights.length < 3) {
      return new Response(
        JSON.stringify({ error: "Pas assez de lights qualifiées (minimum 3)" }),
    ...
    // Simule le pipeline (dans un vrai système : job async lourd)
    ...
    // Simule la complétion du job (dans un vrai système : délégué à un worker)
    // Crée un master placeholder avec une image générée
    const masterUrl = `https://via.placeholder.com/1920x1080/0a0a1a/4fc3f7?text=${encodeURIComponent(`${object_id} — ${lights.length} frames`)}`;
    ...
    // Marque les lights comme stackées
    await supabase
      .from("astro_uploads")
