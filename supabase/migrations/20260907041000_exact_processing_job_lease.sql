create or replace function private.lease_processing_job_by_id(
  p_job_id uuid,
  p_worker_id text,
...
begin
  if p_job_id is null
    ...
    raise exception 'invalid exact lease request';
  end if;
...
revoke all on function private.lease_processing_job_by_id(uuid, text, integer)
  from public, anon, authenticated;
grant execute on function private.lease_processing_job_by_id(uuid, text, integer)
  to service_role;

comment on function private.lease_processing_job_by_id(uuid, text, integer) is
  'Leases exactly one non-inline science job by UUID for queue-driven workers.';
